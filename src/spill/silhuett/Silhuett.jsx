...
export default function Silhuett({ onBack }) {
  ...
  useEffect(() => {
    if (!introSpilt.current && runde) {
      ...
      setTimeout(() => speak("Hvem gjemmer seg i skyggen?"), 400);
    }
  }, [runde]);
  ...
  const velg = (dyr) => {
    ...
    if (dyr.id === runde.fasit.id) {
      setFeedback("riktig");
      speak(`Ja! Det er en ${runde.fasit.navn}!`);
      setTimeout(() => {
        setStjerner((s) => s + 1);
        if (rundeIndex + 1 < RUNDER) {
          ...
          setFerdig(true);
          speak("Du gjettet alle skyggene! Godt øye!");
        }
      }, 1900);
    } else {
      setFeedback("feil");
      speak("Nesten! Se godt på formen.");
      setTimeout(() => { setFeedback(null); setValgt(null); }, 1100);
    }
  };

  if (ferdig) return (
    <div className="sil-scene">
      <Sky />
  ...
  return (
    <div className="sil-scene">
      ...
      <div className="sil-stjerner">
        {"⭐".repeat(stjerner)}{"☆".repeat(RUNDER - stjerner)}
      </div>
      ...
      <div className="sil-boks" style={{ borderColor: runde.fasit.kant }}>
        <span
          className={"sil-emoji" + (feedback === "riktig" ? " avslort" : "")}
