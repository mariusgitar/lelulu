...
function lagAlternativer(fasit) {
  const kand = new Set();
  for (const f of [0.3, 0.5, 0.7, 1.4, 2.0, 2.8, 0.2, 1.8])
    kand.add(Math.max(1, Math.round(fasit * f)));
...
export default function Dyredetektiven({ onBack }) {
  ...
  const startIdle = () => {
    ...
    idleRef.current = setTimeout(() => {
      if (faseRef.current === "start") {
        speak("Trykk på den gule knappen for å starte!");
      } else if (faseRef.current === "gjett" && valgtRef.current === null && dyrRef.current) {
        const d = dyrRef.current;
        const a = altRef.current;
        speak(`Hvor gammel kan en ${d.navn.toLowerCase()} omtrent bli?`, () => {
          a.forEach((v, i) =>
            setTimeout(() => {
              setOpplestIndeks(i);
              speak(`${v} år`, () => {
                if (i === a.length - 1) { setOpplestIndeks(-1); startIdle(); }
              });
            }, i * 1800)
          );
        });
  ...
  useEffect(() => {
    ...
    return () => { clearIdle(); clearInterval(autoRef.current); };
  ...
  function initRunde(liste, idx) {
    ...
    setTimeout(() => {
      speak(`Hvor gammel kan en ${liste[idx].navn.toLowerCase()} omtrent bli?`, () => {
        alt.forEach((v, i) =>
          setTimeout(() => {
            setOpplestIndeks(i);
            speak(`${v} år`, () => {
              if (i === alt.length - 1) { setOpplestIndeks(-1); startIdle(); }
            });
          }, i * 1800)
        );
      });
  ...
  function velgSvar(val) {
    ...
    const resp = riktig
      ? ["Ja, det stemmer!", "Bra gjetta!", "Helt riktig!"][Math.floor(Math.random() * 3)]
      : p === 2 ? "Nesten! Du var ganske nær!" : "Ikke helt, men du prøvde!";
    speak(`${resp} ${gjeldende.maxAlder} år. Visste du at... ${gjeldende.fakta}`, () => {
      let t = 3;
      setNedtelling(t);
  ...
  function neste(ekstra = 0) {
    ...
    if (indeks + 1 >= RUNDER) {
      ...
      const pst = tot / maks;
      setTimeout(() => speak(
        pst === 1 ? "Du er en ekte dyreekspert!" :
        pst >= 0.7 ? "Du kan mye om dyr!" : "Spill igjen og lær enda mer!"
      ), 400);
    } else {
      const ny = indeks + 1;
      setIndeks(ny);
    ...
    <div className="dd-scene">
      ...
      {fase === "start" && (
        <div className="dd-start">
          ...
          <p className="dd-ingress">
            Hvor gammel kan dyrene omtrent bli?<br />Trykk på riktig svar!
          </p>
      ...
      {fase === "gjett" && gjeldende && (
        <div className="dd-gjett">
          ...
          <p className="dd-sporsmal">
            Hvor gammel kan en {gjeldende.navn.toLowerCase()} omtrent bli?
          </p>
          ...
          {valgt === null && (
            <button className="dd-stopp" onClick={() => {
              clearIdle();
              stoppLyd();
              setOpplestIndeks(-1);
              speak(`Hvor gammel kan en ${gjeldende.navn.toLowerCase()} omtrent bli?`, () => {
                alternativer.forEach((v, i) =>
                  setTimeout(() => {
                    setOpplestIndeks(i);
                    speak(`${v} år`, () => {
                      if (i === alternativer.length - 1) { setOpplestIndeks(-1); startIdle(); }
                    });
                  }, i * 1800)
                );
              });
      ...
      {fase === "ferdig" && (
        <div className="dd-ferdig">
          ...
          <h2 className="dd-tittel">
            {poeng === maks ? "Perfekt!" : poeng >= maks * 0.7 ? "Bra jobba!" : "Godt forsøk!"}
          </h2>
