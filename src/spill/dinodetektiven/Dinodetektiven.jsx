...
function DinoBilde({ dino, stor = false }) {
  ...
  return <img src={dino.img} alt={dino.navn}
...
export default function Dinodetektiven({ onBack }) {
  ...
  const runder = useMemo(() => {
    ...
    return shuffled.map((fasit) => {
      const andre = DINO.filter((d) => d.id !== fasit.id).sort(() => Math.random() - 0.5).slice(0, 2);
  ...
  const lesBæsj = async () => {
    if (!runde) return;
    await unlockAudio();
    await playAudio(`/lyd/dino/${runde.fasit.id}_baesj.mp3`, `Hvem bæsjer ${runde.fasit.bæsjBeskrivelse}`);
  };
  ...
  const startNedtelling = (ekstraPoeng) => {
    ...
    autoRef.current = setInterval(() => {
      t -= 1;
      if (t <= 0) { clearInterval(autoRef.current); setNedtelling(null); neste(ekstraPoeng); }
      else setNedtelling(t);
    }, 1000);
  };

  const velg = async (dino) => {
    if (feedback) return;
    setValgt(dino.id);
    if (dino.id === runde.fasit.id) {
      ...
      setPoeng((p) => p + 1);
      setVisFakta(true);
      // Spill riktig-klipp (inneholder "Riktig! Det er X! + fakta1"), deretter fakta2
      await playAudio(
        `/lyd/dino/${runde.fasit.id}_riktig.mp3`,
        `Riktig! Det er ${runde.fasit.navn}! ${runde.fasit.fakta}`
      );
  ...
  const neste = (ekstra = 0) => {
    ...
    if (rundeIndex + 1 < RUNDER) {
      ...
      setFerdig(true);
      playAudio(
        tot >= RUNDER - 1 ? "/lyd/fraser/dino_ferdig_bra.mp3" : "/lyd/fraser/dino_ferdig_ok.mp3",
        tot >= RUNDER - 1 ? "Du er en ekte dino-ekspert!" : "Bra innsats! Spill igjen og lær mer om dinosaurene!"
      );
    }
  };
  ...
  if (fase === "start") return (
    <div className="dino-scene">
      ...
      <div className="dino-start">
        ...
        <p className="dino-ingress">Hvem bæsjet dette? Gjett hvilken dinosaur bæsjen tilhører!</p>
        <button className="dino-btn" onClick={async () => {
          await unlockAudio();
          setFase("gjett");
        }}>🔍 Start!</button>
      </div>
        ...
        {runde.alternativer.map((dino) => {
          ...
          return (
            <button key={dino.id}
              ...
              onClick={() => velg(dino)} disabled={!!feedback}>
              <DinoBilde dino={dino} />
