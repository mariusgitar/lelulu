...
export default function LydDetektiven({ onBack }) {
  ...
  const velg = (dyr) => {
    ...
    if (dyr.id === runde.fasit.id) {
      setFeedback("riktig");
      speak(`Riktig! Det var ${runde.fasit.navn}en!`);
      setTimeout(() => {
        setStjerner((s) => s + 1);
        if (rundeIndex + 1 < RUNDER) {
          ...
          setFerdig(true);
          speak("Du kjente igjen alle dyrelydene! Du er en ekte lyddetektiv!");
        }
      }, 1700);
    } else {
      setFeedback("feil");
      speak("Lytt en gang til!");
  ...
  return (
    <div className="lyd-scene">
      ...
      <button
        className={"lyd-stor-knapp" + (spillerLyd ? " spiller" : "")}
        onClick={lesLyd}
